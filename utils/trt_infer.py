...
def allocate_buffers_v2(context):
    inputs = []
    outputs = []
    bindings = []
    stream = cuda.Stream()
    for idx, binding in enumerate(context.engine):
        # size = trt.volume(engine.get_binding_shape(binding)) * engine.max_batch_size
        size = trt.volume(context.get_binding_shape(idx))
        dtype = trt.nptype(context.engine.get_binding_dtype(idx))
        # Allocate host and device buffers
        host_mem = cuda.pagelocked_empty(size, dtype)
        device_mem = cuda.mem_alloc(host_mem.nbytes)
        # Append the device buffer to device bindings.
...
class EngineBuilder_v2:
    ...
    def create_network(self, onnx_path, get_inputs):
        """
        Parse the ONNX graph and create the corresponding TensorRT network definition.
        :param onnx_path: The path to the ONNX graph to load.
        """
        ...
        inputs = []
        for i, shape in enumerate(get_inputs):
            self.network.get_input(i).shape = shape
            inputs.append(shape)
        # inputs = [self.network.get_input(i) for i in range(self.network.num_inputs)]
        outputs = [self.network.get_output(i) for i in range(self.network.num_outputs)]
