...
from utils_detection import yaml_load, letterbox_image, scale_bboxes, non_max_suppression, \
...
class yolox_engine_det():
    ...
    def draw(self, frame):
        ...
        for i in pred:
            ...
            # frame = cv.rectangle(frame, bbox[:2], bbox[2:], thickness=2, color=self.colors[i[-1]])
            # frame = cv.putText(frame, f'{self.labels[i[-1]]}:{i[-2]:.2f}', (bbox[0] + 5, bbox[1] + 30),
            #                    fontFace=cv.FONT_HERSHEY_DUPLEX, fontScale=1, color=(255, 126, 0))
