...
void preprocess(cv::Mat& image, preproc_struct& image_trans, const cv::Size& resize)
{
    ...
    cv::resize(image, image_resize, cv::Size(w, h), 0, 0, 2);
    ...
    //imgresize(image, image_resize, scale, resize);

    int size = image_resize.total();
    /*image_trans.img = new float[size * 3];*/
    //float*  img_trans = image_trans.img;
...
{
    box.x = cv::max(0., (box.x - (double)preproc_res.w_p)) / preproc_res.scale;
    box.y = cv::max(0., (box.y - (double)preproc_res.h_p)) / preproc_res.scale;
